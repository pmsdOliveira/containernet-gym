...
class ContainernetBackend(object):
    def __init__(self, graph: dict = None):
        ...
        self.hosts_switches_from_graph()
        ...
        self.links_switch_port_mapping()
        print("Created %s links." % len(self.net.links))

        shortest_paths = self.find_all_shortest_paths()
        self.net.start()
        self.add_flows(shortest_paths)
        self.net.pingAll()
        self.net.stop()
    ...
    def hosts_switches_from_graph(self) -> None:
    ...
    def links_switch_port_mapping(self) -> None:
        for vertex in self.graph:
    ...
    def find_all_shortest_paths(self):
        ...
        for origin in range(1, len(self.net.hosts) + 1):
            for destination in range(origin, len(self.net.hosts) + 1):
                shortest_paths.append(self.find_shortest_path('h%s' % origin, 'h%s' % destination))
        return shortest_paths

    def add_flows(self, shortest_paths):
        for switch in range(len(self.net.switches)):
            switch_name: str = 's%s' % (switch + 1)
            cmd: str = 'ovs-ofctl --protocols=OpenFlow13 add-flow %s ' % switch_name
            for host in range(len(self.net.hosts)):
